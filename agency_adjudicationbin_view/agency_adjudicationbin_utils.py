from video.models import AdjudicationBin
...
class AgencyAdjudicationBinUtils:

    @staticmethod
    def save_agency_adjudication_bin_data(**kwargs):

        if kwargs['video_object']:
            adjudication_bin = AdjudicationBin.objects.filter(video=kwargs.get('video_object')).first()

            if adjudication_bin:
                adjudication_bin.image = kwargs.get('image_object')
                adjudication_bin.video = kwargs.get('video_object')
                adjudication_bin.license_plate = kwargs.get('license_plate', '')
                adjudication_bin.vehicle_state = kwargs.get('vehicle_state', '')
                adjudication_bin.is_submitted = kwargs.get('is_submitted', False)
                adjudication_bin.is_adjudicated_in_adjudicationbin = kwargs.get('is_adjudicated_in_adjudicationbin', False)
                adjudication_bin.is_skipped = kwargs.get('is_skipped', False)
                adjudication_bin.is_sent = kwargs.get('is_sent', False)
                adjudication_bin.is_rejected = kwargs.get('is_rejected', False)
                adjudication_bin.station = kwargs['station_name']
                adjudication_bin.note = kwargs.get('note', '')
                adjudication_bin.tattile = kwargs.get("tattile_object")
                adjudication_bin.save()
                return adjudication_bin

        if kwargs['image_object']:
            adjudication_bin = AdjudicationBin.objects.filter(image=kwargs.get('image_object')).first()
        ...
        if kwargs['tattile_object']:
            adjudication_bin = AdjudicationBin.objects.filter(tattile=kwargs.get('tattile_object')).first()
        ...
        adjudication_bin = AdjudicationBin(
            ...
            is_submitted=kwargs.get('is_submitted', False),
            is_adjudicated_in_adjudicationbin=kwargs.get('is_adjudicated_in_adjudicationbin', False),
            is_skipped=kwargs.get('is_skipped', False),
            is_sent=kwargs.get('is_sent', False),
            is_rejected=kwargs.get('is_rejected', False),
            note=kwargs.get('note', ''),
            station=kwargs['station_name'],
            tattile=kwargs.get('tattile_object')
        )
        adjudication_bin.save()
        return adjudication_bin
    @staticmethod
    def extract_fields(validated_data):
        ...
        return {
            ...
            "is_adjudicated": validated_data.get('isAdjudicatedInAgencyAdjudicationBin', False),
            "is_rejected": validated_data.get('isRejectedInAgencyAdjudicationBin', False),
    ...
    def get_date_object(fields):
        ...
        if fields['mediaType'] == 1:
            ...
            tattile_data = Tattile.objects.filter(id=fields['image_id']).values_list('image_time', flat=True).first()
    ...
    @staticmethod
    def save_citation_data(station_name,station_id, citation_id, person, vehicle, court_date, date_object, fields, speed_pic, plate_pic):
        """
...
def update_existing_citation_data(fields, citationID, mediaType, videoId=None, imageId=None, 
                                  isAdjudicated=False, isSent=False, personId=None, vehicleId=None, station_id=None, tattileId=None):
...
def generate_s3_file_name(media_type, media_id, station_id, speed_pic=None, plate_pic=None):
    ...
    if media_type == 1:
        ...
        if speed_pic:
            file_data = base64.b64decode(speed_pic)
            file_obj = io.BytesIO(file_data)
            file_name = f"{tattile_data.ticket_id}-{formatted_date}.png"
            speed_pic_url = upload_to_s3(file_obj, file_name, "PGM2/speed")
            return speed_pic_url
            ...
            file_data = base64.b64decode(plate_pic)
            file_obj = io.BytesIO(file_data)
            file_name = f"{tattile_data.ticket_id}-{formatted_date}.jpg"
            plate_pic_url = upload_to_s3(file_obj, file_name, "PGM2/plates")
            return plate_pic_url
    else:
